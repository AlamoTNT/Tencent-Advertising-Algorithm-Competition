# -*- coding: utf-8 -*-
"""
Created on Mon May 18 23:05:12 2020
...
data_feature = pd.read_csv('../model_/corpus/texts_ad.csv', index_col=[0])
...
for i, (tr, va) in enumerate(kfold.split(train_feature, score)):
    ...
    clf = LogisticRegression(random_state=2020, C=8)
...
for i in range(stack.shape[1]):
    df_stack['ad_id_tfidf_lr_classfiy_{}'.format(i)] = np.around(stack[:, i], 6) #6是decimals参数, 指保留6位小数
df_stack.to_csv(path+'feature/ad_id_tfidf_lr_error_single_classfiy.csv', index=None, encoding='utf8')
print('lr特征已保存\n')
...
for i, (tr, va) in enumerate(kfold.split(train_feature, score)):
    print('stack:%d/%d' % ((i + 1), n_folds))
    sgd = SGDClassifier(random_state=2020, loss='log')
...
df_stack = pd.DataFrame()
for i in range(stack.shape[1]):
    df_stack['ad_id_tfidf_sgd_classfiy_{}'.format(i)] = np.around(stack[:, i], 6)
df_stack.to_csv(path+'feature/ad_id_tfidf_sgd_error_single_classfiy.csv', index=None, encoding='utf8')
print('sgd特征已保存\n')
...
for i, (tr, va) in enumerate(kfold.split(train_feature, score)):
    print('stack:%d/%d' % ((i + 1), n_folds))
    pac = PassiveAggressiveClassifier(random_state=2020)
...
df_stack = pd.DataFrame()
for i in range(stack.shape[1]):
    df_stack['ad_id_tfidf_pac_classfiy_{}'.format(i)] = np.around(stack[:, i], 6)
df_stack.to_csv(path+'feature/ad_id_tfidf_pac_error_single_classfiy.csv', index=None, encoding='utf8')
print('pac特征已保存\n')
...
for i, (tr, va) in enumerate(kfold.split(train_feature, score)):
    print('stack:%d/%d' % ((i + 1), n_folds))
    ridge = RidgeClassifier(random_state=2020)
...
df_stack = pd.DataFrame()
for i in range(stack.shape[1]):
    df_stack['ad_id_tfidf_ridge_classfiy_{}'.format(i)] = np.around(stack[:, i], 6)
df_stack.to_csv(path+'feature/ad_id_tfidf_ridge_error_single_classfiy.csv', index=None, encoding='utf8')
print('ridge特征已保存\n')
...
df_stack = pd.DataFrame()
for i in range(stack.shape[1]):
    df_stack['ad_id_tfidf_bnb_classfiy_{}'.format(i)] = np.around(stack[:, i], 6)
df_stack.to_csv(path+'feature/ad_id_tfidf_bnb_error_single_classfiy.csv', index=None, encoding='utf8')
...
df_stack = pd.DataFrame()
for i in range(stack.shape[1]):
    df_stack['ad_id_tfidf_mnb_classfiy_{}'.format(i)] = np.around(stack[:, i], 6)
df_stack.to_csv(path+'feature/ad_id_tfidf_mnb_error_single_classfiy.csv', index=None, encoding='utf8')
...
for i, (tr, va) in enumerate(kfold.split(train_feature, score)):
    print('stack:%d/%d' % ((i + 1), n_folds))
    lsvc = LinearSVC(random_state=2020)
...
df_stack = pd.DataFrame()
for i in range(stack.shape[1]):
    df_stack['ad_id_tfidf_lsvc_classfiy_{}'.format(i)] = np.around(stack[:, i], 6)
df_stack.to_csv(path+'feature/ad_id_tfidf_lsvc_error_single_classfiy.csv', index=None, encoding='utf8')
print('LSVC特征已保存\n')
...
def get_cluster(num_clusters):
    ...
    model = KMeans(n_clusters=num_clusters, max_iter=300, n_init=1, \
                        init='k-means++', n_jobs=10, random_state=2020)
...
get_cluster(60)
get_cluster(70)
kmeans_result.to_csv(path+'feature/ad_id_cluster_tfidf_feature.csv', index=False)
