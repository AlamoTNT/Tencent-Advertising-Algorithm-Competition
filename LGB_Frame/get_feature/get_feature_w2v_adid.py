# -*- coding: utf-8 -*-
"""
Created on Mon May 18 11:16:30 2020
...
path = '../data/'
train_ad_df = pd.read_csv(path+'train_preliminary/ad.csv', usecols=['creative_id', 'ad_id'])
...
#提取用户点击的广告序列, 并构成文本
doc = whole_click_ad_df.groupby(['user_id'])['ad_id'].agg({list}).reset_index()
...
w2v_model.wv.save_word2vec_format('./word2vec_model/adid_w2v_128.txt')
del texts
...
def get_w2v_avg(doc, w2v_out_path, word2vec_Path):
    ...
    w2v_avg.columns = ['w2v_avg_adid_' + str(i) for i in range(1, w2v_dim+1)]
...
w2v_feat = get_w2v_avg(doc, path+'feature/adid_w2v_avg_feature.csv', './word2vec_model/adid_w2v_128.txt')