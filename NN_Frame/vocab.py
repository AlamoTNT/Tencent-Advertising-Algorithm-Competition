...
class VocabEntry(object):
    ...
    def __getitem__(self,word):
        """获取word的idx"""
        #存在word, 返回self.word2id[word], 否则返回self.unk_id
...
if __name__ == '__main__':
    ...
    src_sents, labels = read_corpus('./work/corpus/')
    labels = {label:idx for idx,label in enumerate(labels)}
    
    SIZE = 3000000  #选取常用creative_id的数目
    MIN_FEQ = 3
    vocab = Vocab.build(src_sents, labels, SIZE, MIN_FEQ)
